...
- As a content admin, I want backend Document exports to include a column for each Place relationship type, so that I can easily see and analyze geographic information
- As a content editor, I want the csv of people-doc relations to include admin and public URLs to the PGPIDs so that I can easily click into the records from the csv.
- As a content editor, I would like LtR scripts to be compatible with the transcription editor, so that I can enter Latin-script transcriptions
- bugfix: Fix the repeated columns generated by people export
- chore: Correcting the license link in the readme
- chore: Correcting the unit tests badge on the readme to display success based on the stable (main) branch only
