"""
Script to consolidate redundant or duplicate document records.
The script has two modes:

**Report** mode looks for merge candidates based on duplicate
shelfmark combinations, document type, and descriptions. To generate
a report of potential merges and actions to be taken::

    python manage.py. merge_documents report

**Merge** mode takes a CSV file in the same format generated by the report
and merges documents as specified. There should be one row for each document
that is part of any group of documents to be merged. Required fields are:
