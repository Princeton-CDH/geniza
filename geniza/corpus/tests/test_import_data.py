...
def test_get_csv_error(mockrequests):
    ...
    mockrequests.get.assert_called_with(
        settings.DATA_IMPORT_URLS["libraries"], stream=True
    )
...
def test_get_csv_success():
    ...
    with patch.object(requests, "get") as mockget:
        ...
        mockget.return_value.iter_lines.return_value = iter(
            [b"Library,Abbreviation", b"British Library,BL", b"Bodleian Library,BODL"]
        )
...
def test_import_collections(mockrequests):
    ...
    assert (
        LogEntry.objects.filter(action_flag=ADDITION)[0].change_message
...
def test_get_iiif_url():
    ...
    assert (
        import_data_cmd.get_iiif_url(data)
...
def test_get_fragment():
    ...
    import_data_cmd.library_lookup = {"CUL": Collection.objects.create(library="CUL")}
    ...
    assert LogEntry.objects.get(
        action_flag=ADDITION, object_id=newfrag.pk, content_type_id=fragment_ctype.pk
    )
...
def test_add_document_language(caplog):
    ...
    import_data_cmd.language_lookup = {"arabic": arabic, "hebrew": hebrew}

    doc = Document.objects.create()

    row = AttrMap(
        {"pgpid": "3550", "language": "Hebrew? (Tiberian vocalisation); arabic"}
    )
...
def test_get_edit_history(caplog):
    ...
    history = get_edit_history(
        "Sarah Nisenson; Emily Silkaitis", "3/16/2018; 3/23/21"
    )  # two-digit year
    assert history[0]["type"] == ADDITION
    assert history[0]["user"].username == "snisenson"
    assert history[0]["date"] == datetime.date(2018, 3, 16)
    ...
    history = get_edit_history(
        "Emily Silkaitis", "3/16/2018; 03/2021"
    )  # missing day value
    assert history[0]["type"] == ADDITION
    assert history[0]["user"].username == import_data_cmd.team_user.username
    assert history[0]["date"] == datetime.date(2018, 3, 16)
    ...
    with caplog.at_level(logging.DEBUG, logger="import"):
        history = get_edit_history(
            "Amir Ashur and Oded Zinger; Emily Silkaitis", "8/1/2017; November 2020"
        )  # literal month
        assert "found coauthored event" in caplog.record_tuples[-1][2]
        assert history[0]["type"] == ADDITION
        assert history[0]["user"].username == "aashur"
...
def test_log_edit_history():
    ...
    entries = LogEntry.objects.filter(object_id=doc1.pk, content_type_id=dtype.pk)
    ...
    log_edit_history(
        doc2, [{"type": ADDITION, "user": user, "date": date, "orig_date": "test"}]
    )
    entries = LogEntry.objects.filter(object_id=doc2.pk, content_type_id=dtype.pk)
    ...
    assert (
        entries[1].get_change_message()
        == "Initial data entry (spreadsheet), dated test"
    )
    ...
    entries = LogEntry.objects.filter(object_id=doc3.pk, content_type_id=dtype.pk)
...
def test_command_line():
    ...
    with patch.multiple(
        "geniza.corpus.management.commands.import_data.Command",
        import_collections=DEFAULT,
        import_languages=DEFAULT,
        import_documents=DEFAULT,
    ) as mock:
...
editors_parsed = [
    ("Ed. M. Cohen", [{"get_source_arg": "M. Cohen"}]),
    (
    ...
    (
        ...
        [{"get_source_arg": "Gil, Palestine, vol. 2", "f_location": "#177"}],
    ),
    (
...
source_input = [
    ...
    ("M. Cohen", {"type": "Unpublished", "authors": ["Cohen, Mark"]}),
    (
    ...
    (
        ...
        {"type": "Unpublished", "authors": ["Goitein, S. D."], "title": "India Book 6"},
    ),  # notes/location?
    # typed texts
    (
        "Goitein, typed texts",
        {"type": "Unpublished", "authors": ["Goitein, S. D."], "title": "typed texts"},
    ),
    ...
    (
        "Gil, Kingdom Vol 3",
        {"type": "Book", "authors": ["Gil, Moshe"], "title": "Kingdom", "volume": "3"},
    ),
    # dissertation
    (
...
def test_get_source(test_input, expected):
    ...
    assert expected.get("language", "") == ", ".join(
        lang.name for lang in source.languages.all()
    )
...
def test_get_source_existing(source):
    ...
    import_data_cmd.source_types = {s.type: s for s in SourceType.objects.all()}
    # create source creator lookup keyed on last name
    import_data_cmd.source_creators = {c.last_name: c for c in Creator.objects.all()}
    import_data_cmd.content_types = {Source: ContentType.objects.get_for_model(Source)}
    import_data_cmd.script_user = get_user_model().objects.get(
